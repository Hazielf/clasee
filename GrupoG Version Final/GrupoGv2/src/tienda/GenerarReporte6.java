...
public class GenerarReporte6 extends JDialog implements ActionListener {
	...
	double calcularPrecioMaximo() {
	    ...
	    // Recorremos el arreglo para encontrar el precio mÃ¡s alto
	    for (int i = 1; i < precio.length; i++) { // Comenzamos desde 1 porque ya asumimos que el 0 es el mÃ¡ximo
	        if (precio[i] > maxPrecio) {
	            maxPrecio = precio[i]; // Actualizamos el precio mÃ¡ximo
	        }
	    }
	    return maxPrecio; // Retornamos el precio mÃ¡s alto
	}
